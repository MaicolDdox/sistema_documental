<x-app-layout>
    ...
    <nav class="flex mb-4 text-sm text-slate-500" aria-label="Breadcrumb">
        <a href="{{ route('director-grupo-investigacion.dashboard') }}" class="hover:text-slate-700">Director de Grupo</a>
        <span class="mx-2">/</span>
        <a href="{{ route('director-grupo-investigacion.minciencias.index') }}" class="hover:text-slate-700">Productos Minciencias</a>
        <span class="mx-2">/</span>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-700">{{ $producto->fecha_inicio?->format('d/m/Y') ?? '—' }} - {{ $producto->fecha_fin?->format('d/m/Y') ?? '—' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
            ...
            <ul class="divide-y divide-slate-100">
                ...
                <li class="px-5 py-3 flex items-center justify-between text-sm gap-3">
                    ...
                    @if($archivo->archivo)
                    <a href="{{ route('director-grupo-investigacion.minciencias.archivos.descargar', $archivo) }}" class="text-[#39A900] hover:underline text-xs font-medium">Descargar</a>
                    @endif
                </li>
                @empty
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5">
            ...
            <div class="flex items-center gap-3">
                <form action="{{ route('director-grupo-investigacion.minciencias.aprobar', $producto) }}" method="POST">
                    @csrf
            ...
            <form id="rechazo-form" action="{{ route('director-grupo-investigacion.minciencias.rechazar', $producto) }}" method="POST" class="hidden mt-4">
                @csrf
