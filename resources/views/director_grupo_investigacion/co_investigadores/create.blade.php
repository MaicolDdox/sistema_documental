...
@section('title', 'Nuevo Co-investigador GDI')
@section('header', '')

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="mb-5">
        <a href="{{ route('director-grupo-investigacion.co-investigadores.index') }}" class="text-sm text-slate-500 hover:text-[#39A900] flex items-center gap-1 transition-colors">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50">
            <h3 class="text-base font-semibold text-slate-900">Datos del nuevo Co-investigador GDI</h3>
            <p class="text-xs text-slate-500 mt-0.5">Quedará vinculado automáticamente a tu grupo de investigación.</p>
        </div>

        <form action="{{ route('director-grupo-investigacion.co-investigadores.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="border-t border-slate-100 pt-5 flex items-center justify-end gap-3">
                <a href="{{ route('director-grupo-investigacion.co-investigadores.index') }}" class="border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-semibold py-2.5 px-4 rounded-lg text-sm transition-all">Cancelar</a>
