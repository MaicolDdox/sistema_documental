...
@section('header', 'Crear Semillero')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50">
            ...
            <p class="text-xs text-slate-500 mt-1">Ingresa los datos básicos para registrar un nuevo semillero en el centro de formación.</p>
        </div>
        ...
        <form action="{{ route('dir-sem.semilleros.store') }}" method="POST" class="p-6">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="nombre" id="nombre" value="{{ old('nombre') }}" required placeholder="Ej: Semillero de Desarrollo de Software"
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-2">Solo se muestran usuarios activos con el rol <span class="font-medium">Líder de Semillero</span> de este centro.</p>
                </div>

                <!-- Descripción -->
                <div>
                    <label for="descripcion" class="block text-sm font-medium text-slate-700 mb-1.5">Descripción o Enfoque (Opcional)</label>
                    <textarea name="descripcion" id="descripcion" rows="4" placeholder="Describe brevemente el propósito, las líneas de investigación asociadas, etc."
                              class="w-full border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm text-slate-800 focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10 transition-all">{{ old('descripcion') }}</textarea>
