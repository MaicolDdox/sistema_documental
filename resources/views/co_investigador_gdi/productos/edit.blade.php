...
<div class="sgd-card bg-white rounded-xl border border-slate-200 shadow-sm p-6">
    <form action="{{ route('co-investigador-gdi.productos.update', $producto) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('co-investigador-gdi.productos.show', $producto) }}" class="text-sm font-medium text-slate-500 hover:text-slate-700">Cancelar</a>
        </div>
    </form>
</div>
@endsection
