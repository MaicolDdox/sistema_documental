...
<div class="mb-6 flex items-start justify-between gap-4">
    <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">Grupo: {{ $producto->grupoInvestigacion?->nombre ?? '—' }}</p>
    </div>
    <div class="flex items-center gap-3 shrink-0">
        <a href="{{ route('co-investigador-gdi.productos.edit', $producto) }}"
        ...
        <a href="{{ route('co-investigador-gdi.productos.index') }}"
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5">
        ...
        <form action="{{ route('co-investigador-gdi.productos.archivos.store', $producto) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @csrf
            <div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <ul class="divide-y divide-slate-100">
            ...
            <li class="px-5 py-3 flex items-center justify-between text-sm gap-3">
                ...
                <div class="flex items-center gap-3 shrink-0">
                    @if($archivo->archivo)
                    <a href="{{ route('co-investigador-gdi.archivos.descargar', $archivo) }}" class="text-[#39A900] hover:underline text-xs font-medium">Descargar</a>
                    @endif
                    <form action="{{ route('co-investigador-gdi.archivos.destroy', $archivo) }}" method="POST" onsubmit="return confirm('¿Eliminar este archivo?');">
                        @csrf @method('DELETE')
