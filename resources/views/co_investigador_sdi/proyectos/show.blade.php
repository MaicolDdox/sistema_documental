...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5">
        ...
        <form action="{{ route('co-investigador-sdi.proyectos.evidencias.store', $proyecto) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @csrf
            <div>
        ...
        <ul class="divide-y divide-slate-100">
            ...
            <li class="px-5 py-3 flex items-center justify-between text-sm gap-3">
                ...
                <div class="flex items-center gap-3 shrink-0">
                    @if($ev->archivo)
                    <a href="{{ route('co-investigador-sdi.evidencias.descargar', $ev) }}" class="text-[#39A900] hover:underline text-xs font-medium">Descargar</a>
                    @endif
                    @if((int) $ev->uploaded_by === auth()->id())
                    <form action="{{ route('co-investigador-sdi.evidencias.destroy', $ev) }}" method="POST" onsubmit="return confirm('¿Eliminar esta evidencia?');">
                        @csrf @method('DELETE')
