...
<head>
    <meta charset="UTF-8">
    <title>{{ $titulo ?? 'Proyectos del Semillero' }}</title>
    <style>
...
<body>
    <div class="header">
        <p class="title">{{ $titulo ?? 'Proyectos del Semillero' }}</p>
    ...
    <table class="summary">
        <tr>
            <td><strong>Total proyectos:</strong> {{ $resumen['total_proyectos'] ?? 0 }}</td>
        </tr>
    </table>
    @endif
    <table>
        <thead>
            <tr>
                ...
                <th>Estado</th>
                <th>Evid. Desarrollo</th>
                <th>Evid. Producto Final</th>
            </tr>
        </thead>
        <tbody>
            @forelse($filas ?? [] as $f)
            <tr>
                ...
                <td>{{ $f['estado'] }}</td>
                <td>{{ $f['evidencias_desarrollo'] }}</td>
                <td>{{ $f['evidencias_producto_final'] }}</td>
            </tr>
            @empty
