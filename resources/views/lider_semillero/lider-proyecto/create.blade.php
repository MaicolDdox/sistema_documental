...
@section('title', 'Nuevo Líder de Proyecto')
@section('header', '')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-5 flex items-center justify-between">
        <a href="{{ route('lider-sem.lider-proyecto.index') }}" class="text-sm text-slate-500 hover:text-[#39A900] flex items-center gap-1 transition-colors">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center gap-3">
            ...
            <div>
                <h3 class="text-base font-semibold text-slate-900 font-heading">Datos del nuevo Líder de Proyecto</h3>
                <p class="text-xs text-slate-500 mt-0.5">La contraseña temporal se generará automáticamente.</p>
            </div>
        </div>

        <form action="{{ route('lider-sem.lider-proyecto.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="mb-6 p-4 bg-green-50 border border-green-100 rounded-lg flex items-start gap-3">
                ...
                <div class="text-sm">
                    ...
                    <p class="text-slate-500 mt-0.5">La contraseña temporal siempre se genera para entregarla manualmente. Si activas esta opción, además se enviará por correo.</p>
                </div>
            </div>

            <div class="border-t border-slate-100 pt-5 flex items-center justify-end gap-3">
                <a href="{{ route('lider-sem.lider-proyecto.index') }}"
