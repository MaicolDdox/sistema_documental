...
<div class="max-w-3xl" x-data="{
    semilleroId: '{{ old('seedling_id', '') }}',
    tienesMacro: {{ old('tiene_macroproyecto', '0') }} == 1,
        ...
        <form method="POST" action="{{ route('asesor.proyectos.store') }}" novalidate>
            ...
            <div class="mb-5">
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div>
                        ...
                        <textarea name="descripccion" rows="3" class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10 transition-all resize-none">{{ old('descripccion') }}</textarea>
                    </div>
                </div>
            </div>
                    ...
                    <div>
                        ...
                        <select name="research_line_id" class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10 transition-all @error('research_line_id') border-red-400 @enderror">
                            ...
                            @foreach($lineasInves as $l)
                                <option value="{{ $l->id }}" {{ old('research_line_id') == $l->id ? 'selected' : '' }}>{{ $l->nombre }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="technological_line_id" class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10 transition-all">
                            ...
                            @foreach($lineasTec as $lt)
                                <option value="{{ $lt->id }}" {{ old('technological_line_id') == $lt->id ? 'selected' : '' }}>{{ $lt->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="thematic_area_id" class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10 transition-all">
                            ...
                            @foreach($areasTematicas as $at)
                                <option value="{{ $at->id }}" {{ old('thematic_area_id') == $at->id ? 'selected' : '' }}>{{ $at->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="project_modality_id" class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10 transition-all @error('project_modality_id') border-red-400 @enderror">
                            ...
                            @foreach($modalidades as $m)
                                <option value="{{ $m->id }}" {{ old('project_modality_id') == $m->id ? 'selected' : '' }}>{{ $m->nombre }}</option>
                            @endforeach
                        </select>
                        @error('project_modality_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Tipo de investigación <span class="text-red-500">*</span></label>
                        <select name="investigation_type_id" class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10 transition-all @error('investigation_type_id') border-red-400 @enderror">
                            <option value="">Seleccionar...</option>
                            @foreach($tiposInves as $ti)
                                <option value="{{ $ti->id }}" {{ old('investigation_type_id') == $ti->id ? 'selected' : '' }}>{{ $ti->nombre }}</option>
                            @endforeach
                        </select>
            ...
            <div class="mb-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Fecha de fin (estimada)</label>
                ...
                <div class="flex items-center gap-3 mb-4">
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" name="tiene_macroproyecto" value="1"
                               class="sr-only peer"
                               {{ old('tiene_macroproyecto') ? 'checked' : '' }}
                    ...
                    <span class="text-sm text-slate-700">¿El proyecto está vinculado a un macroproyecto?</span>
                </div>
                <div x-show="tienesMacro" class="grid grid-cols-1 gap-4" style="display: none;">
                    <div>
                        ...
                        <select name="macro_project_id" class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10 transition-all @error('macro_project_id') border-red-400 @enderror">
                            ...
                            <template x-for="mp in macrosFiltradas" :key="mp.id">
                                <option :value="mp.id" x-text="'[' + mp.codigo + '] ' + mp.nombre" :selected="mp.id == '{{ old('macro_project_id') }}'"></option>
                            </template>
                        </select>
                        @error('macro_project_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                </div>
                <p class="text-xs text-slate-400 mt-1">El macroproyecto debe estar registrado en el grupo de investigación vinculado al semillero.</p>
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <a href="{{ route('asesor.proyectos.index') }}" class="px-5 py-2.5 rounded-lg text-sm font-medium text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 transition-all">Cancelar</a>
            </div>
        </form>
    </div>
</div>
