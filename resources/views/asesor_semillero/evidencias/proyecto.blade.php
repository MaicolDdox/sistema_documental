...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5">
        ...
        <form method="POST" action="{{ route('asesor.evidencias.proyecto.store', $proyecto->id) }}" enctype="multipart/form-data" novalidate>
            @csrf
            <div class="space-y-3">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Nombre / descripción <span class="text-red-500">*</span></label>
                    <input type="text" name="nombre" value="{{ old('nombre') }}" placeholder="Ej: Acta reunión 2026-03-01"
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5">
        ...
        @if($evidencias->isEmpty())
            <p class="text-sm text-slate-400">Sin evidencias subidas para este proyecto.</p>
        @else
        <div class="space-y-3">
            ...
            <div class="flex items-start justify-between gap-3 py-3 border-b border-slate-100 last:border-0">
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    <form method="POST" action="{{ route('asesor.evidencias.destroy', $ev->id) }}"
                          onsubmit="return confirm('¿Eliminar esta evidencia? No se puede deshacer.')">
                        @csrf
                        @method('DELETE')
