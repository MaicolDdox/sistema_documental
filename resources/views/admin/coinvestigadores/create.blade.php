<x-app-layout>
    <x-slot name="header">Crear Co-investigador</x-slot>
<div class="mb-6 flex items-center justify-between">
    <div>
        <h2 class="text-xl font-semibold text-slate-900">Crear Co-investigador</h2>
        <p class="text-sm text-slate-500 mt-1">Este formulario crea usuarios únicamente con el rol <strong>Co-investigador</strong>.</p>
    </div>
    <a href="{{ route('admin.usuarios.index', ['rol' => 'co_investigador']) }}" class="border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-semibold py-2.5 px-4 rounded-lg text-sm transition-all hidden sm:inline-block">
        Volver al listado
    </a>
</div>
...
<div class="bg-white rounded-xl border border-slate-200 overflow-hidden max-w-4xl">
    ...
    <div class="p-5">
        <form method="POST" action="{{ route('admin.co-investigadores.store') }}" class="space-y-5" x-data="{ tieneMasRoles: {{ old('tiene_mas_roles') ? 'true' : 'false' }} }">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2 rounded-lg border border-slate-200 bg-slate-50/60 px-4 py-3 text-sm text-slate-600">
                    Rol asignado: <strong class="text-slate-800">Co-investigador</strong> — sin centro de formación propio (puede participar en proyectos de cualquier centro).
                </div>
            </div>
            ...
            <div class="rounded-lg border border-slate-200 bg-slate-50/60 px-4 py-3.5">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mt-0.5">Además de Co-investigador, podrá tener funciones activas de otros roles y cambiar entre ellos desde "Mis roles". Si alguno de esos roles exige centro de formación, se le asigna el tuyo.</p>
                    </div>
                </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                <a href="{{ route('admin.usuarios.index', ['rol' => 'co_investigador']) }}" class="border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-semibold py-2.5 px-4 rounded-lg text-sm transition-all">
                    Cancelar
                </a>
