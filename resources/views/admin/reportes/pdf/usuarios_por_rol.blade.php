...
<head>
    <meta charset="UTF-8">
    <title>{{ $titulo ?? 'Usuarios por Rol' }}</title>
    <style>
...
<body>
    <div class="header">
        <p class="title">{{ $titulo ?? 'Usuarios por Rol' }}</p>
    ...
    <table class="summary">
        <tr>
            <td><strong>Total usuarios:</strong> {{ $resumen['total_usuarios'] ?? 0 }}</td>
            <td><strong>Por rol:</strong> {{ $resumen['por_rol'] ?? '—' }}</td>
        </tr>
    </table>
    @endif
    <table>
