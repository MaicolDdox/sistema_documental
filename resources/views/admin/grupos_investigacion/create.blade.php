...
@section('title', 'Nuevo Grupo de Investigación')
@section('header', '')
...
<div class="max-w-2xl mx-auto">
    <nav class="text-sm text-slate-500 mb-2">
        ...
        <span class="text-slate-700 font-medium">Nuevo</span>
    </nav>

    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mt-4">
        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50">
            <h3 class="text-base font-semibold text-slate-900">Datos básicos del grupo</h3>
            <p class="text-xs text-slate-500 mt-0.5">La descripción, el logo y la línea de investigación los completa el director asignado.</p>
        </div>
        <form action="{{ route('admin.grupos-investigacion.store') }}" method="POST" class="p-6 space-y-5">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="text" name="codigo" id="codigo" value="{{ old('codigo') }}" maxlength="50" required
                       class="w-full border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm text-slate-800 focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10">
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="bg-[#39A900] hover:bg-[#2d8500] text-white font-semibold py-2.5 px-4 rounded-lg text-sm transition-all">Guardar Grupo</button>
            </div>
        </form>
    </div>
</div>
