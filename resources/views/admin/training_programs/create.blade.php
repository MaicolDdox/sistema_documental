            ...
            <form method="POST" action="{{ route('admin.training-programs.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="descripcion" id="descripcion" rows="2"
                              class="w-full border @error('descripcion') border-red-500 @else border-slate-200 @enderror rounded-lg px-3.5 py-2.5 text-sm text-slate-800 focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10">{{ old('descripcion') }}</textarea>
                    @error('descripcion')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
