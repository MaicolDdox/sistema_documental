                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-1 flex-wrap">
                                ...
                                @endphp
                                {{-- Ver detalle (modal) --}}
                                <button type="button" @click="openDetalle({{ json_encode($detallePrograma) }})"
                                ...
                                {{-- Editar (modal) --}}
                                <button type="button" @click="openEditar({{ $program->id }}, {{ json_encode($editPrograma) }})"
                                ...
                                <form id="form-delete-program-{{ $program->id }}" method="POST" action="{{ route('admin.training-programs.destroy', $program) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" @click="intentEliminar({{ $estadoActivo ? 'true' : 'false' }}, 'form-delete-program-{{ $program->id }}', {{ json_encode($program->nombre) }})"
                                ...
                                <form method="POST" action="{{ route('admin.training-programs.toggle', $program) }}" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="p-2 rounded-lg text-slate-400 hover:text-[#39A900] hover:bg-green-50 transition-colors" title="{{ $estadoActivo ? 'Desactivar' : 'Activar' }}">
                                        @if($estadoActivo)
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
