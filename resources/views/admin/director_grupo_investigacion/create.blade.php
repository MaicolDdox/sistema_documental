<x-app-layout>
    <x-slot name="header">Crear Director de Grupo de Investigación</x-slot>
<div class="mb-6 flex items-center justify-between">
    <div>
        <h2 class="text-xl font-semibold text-slate-900">Crear Director de Grupo de Investigación</h2>
        <p class="text-sm text-slate-500 mt-1">Este formulario crea usuarios únicamente con el rol <strong>Director de Grupo de Investigación</strong>.</p>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 overflow-hidden max-w-4xl">
    ...
    <div class="p-5">
        <form method="POST" action="{{ route('admin.director-grupo-investigacion.store') }}" class="space-y-5" x-data="{ tieneMasRoles: {{ old('tiene_mas_roles') ? 'true' : 'false' }}, mostrarGrupoInvestigacion: {{ in_array('co_investigador_gdi', old('additional_roles', []), true) ? 'true' : 'false' }} }">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2 rounded-lg border border-slate-200 bg-slate-50/60 px-4 py-3 text-sm text-slate-600">
                    Rol asignado: <strong class="text-slate-800">Director de Grupo de Investigación</strong> — centro de formación: <strong class="text-slate-800">{{ auth()->user()->trainingCenter?->nombre ?? 'tu centro' }}</strong> (heredado automáticamente). Asigna el grupo desde "Vinculaciones grupo de investigación" después de crear la cuenta.
                </div>
            </div>
            ...
            <div class="rounded-lg border border-slate-200 bg-slate-50/60 px-4 py-3.5">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mt-0.5">Además de Director de Grupo de Investigación, podrá tener funciones activas de otros roles y cambiar entre ellos desde "Mis roles".</p>
                    </div>
                </div>
                ...
                <div x-show="tieneMasRoles && mostrarGrupoInvestigacion" x-cloak class="mt-3.5 pt-3.5 border-t border-slate-200">
                    ...
                    <select name="grupo_investigacion_id" id="grupo_investigacion_id" class="w-full border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm text-slate-800 bg-white focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10">
                        ...
                        @foreach($gruposInvestigacion as $g)
                            <option value="{{ $g->id }}" {{ (string) old('grupo_investigacion_id') === (string) $g->id ? 'selected' : '' }}>{{ $g->nombre }}</option>
                        @endforeach
                    </select>
