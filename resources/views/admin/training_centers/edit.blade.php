<x-app-layout>
    <x-slot name="header">Editar Centro de Formación</x-slot>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-xl font-semibold text-slate-900">Editar Centro de Formación</h2>
            <p class="text-sm text-slate-500 mt-1">{{ $training_center->nombre }}</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden max-w-2xl">
        ...
        <div class="p-5">
            <form method="POST" action="{{ route('admin.training-centers.update', $training_center) }}" class="space-y-5">
                @csrf
                @method('PUT')
                <div>
                    <label for="nombre" class="block text-sm font-medium text-slate-700 mb-1.5">Nombre <span class="text-red-500">*</span></label>
                    <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $training_center->nombre) }}" required
                ...
                <div>
                    ...
                    <input type="number" name="codigo" id="codigo" value="{{ old('codigo', $training_center->codigo) }}" min="0" required
                ...
                <div>
                    ...
                    <select name="department_id" id="department_id" required
                        ...
                        @foreach($departments as $dept)
                            <option value="{{ $dept->id }}" {{ old('department_id', $training_center->department_id) == $dept->id ? 'selected' : '' }}>{{ $dept->nombre }}</option>
                        @endforeach
                    </select>
                    @error('department_id')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="city_id" id="city_id" required
                        ...
                        @foreach($cities as $city)
                            <option value="{{ $city->id }}" data-department="{{ $city->department_id }}" {{ old('city_id', $training_center->city_id) == $city->id ? 'selected' : '' }}>{{ $city->nombre }} ({{ $city->department?->nombre }})</option>
                        @endforeach
                    </select>
                    @error('city_id')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
