                        ...
                        <tr class="hover:bg-slate-50/50">
                            ...
                            <td class="px-4 py-2.5 text-right">
                                <div class="flex items-center justify-end gap-1 flex-wrap">
                                    <button type="button" @click="openDetalleA({{ json_encode($detalleA) }})" class="p-1.5 inline-flex text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg" title="Ver detalle">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                    </button>
                                    <button type="button" @click="openEditarA({{ $area->id }}, {{ json_encode($editA) }})" class="p-1.5 inline-flex text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg" title="Editar">
                                    ...
                                    <form id="form-delete-a-{{ $area->id }}" method="POST" action="{{ route('admin.knowledge-areas.destroy', $area) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" @click="intentEliminarA('form-delete-a-{{ $area->id }}', {{ json_encode($area->nombre) }})" class="p-1.5 inline-flex text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg" title="Eliminar">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/50">
                            ...
                            <td class="px-4 py-2.5 text-right">
                                <div class="flex items-center justify-end gap-1 flex-wrap">
                                    <button type="button" @click="openDetalleG({{ json_encode($detalleG) }})" class="p-1.5 inline-flex text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg" title="Ver detalle">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                    </button>
                                    <button type="button" @click="openEditarG({{ $grand->id }}, {{ json_encode($editG) }})" class="p-1.5 inline-flex text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg" title="Editar">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" /></svg>
                                    </button>
                                    <form id="form-delete-g-{{ $grand->id }}" method="POST" action="{{ route('admin.knowledge-grand-areas.destroy', $grand) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" @click="intentEliminarG({{ $hasAreas ? 'true' : 'false' }}, 'form-delete-g-{{ $grand->id }}', {{ json_encode($grand->nombre) }})" class="p-1.5 inline-flex text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg" title="Eliminar">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                                        </button>
                                    </form>
                                </div>
