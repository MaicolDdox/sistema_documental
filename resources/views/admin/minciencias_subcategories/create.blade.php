<x-app-layout>
    <x-slot name="header">Nueva Subcategoría Minciencias</x-slot>
    <nav class="flex mb-4 text-sm text-slate-500" aria-label="Breadcrumb">
        ...
        <span class="mx-2">/</span>
        <span class="text-slate-900 font-medium">Nueva subcategoría</span>
    </nav>
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-xl font-semibold text-slate-900">Nueva Subcategoría</h2>
            <p class="text-sm text-slate-500 mt-1">Vincular a una tipología Minciencias.</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 max-w-xl overflow-hidden">
        ...
        <form method="POST" action="{{ route('admin.minciencias-subcategories.store') }}" class="p-5 space-y-4">
            @csrf
            <div>
