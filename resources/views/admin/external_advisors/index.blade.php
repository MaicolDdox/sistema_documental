                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-1 flex-wrap">
                                <button type="button" @click="openDetalle({{ json_encode($detalle) }})" class="p-2 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Ver detalle">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    </svg>
                                </button>
                                <button type="button" @click="openEditar({{ $advisor->id }}, {{ json_encode($editData) }})" class="p-2 rounded-lg text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition-colors" title="Editar">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                ...
                                <form id="form-delete-advisor-{{ $advisor->id }}" method="POST" action="{{ route('admin.external-advisors.destroy', $advisor) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" @click="intentEliminar('form-delete-advisor-{{ $advisor->id }}', {{ json_encode($advisor->nombre_completo) }})" class="p-2 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Eliminar">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
