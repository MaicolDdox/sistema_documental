<x-app-layout>
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden max-w-3xl">
        ...
        <div class="p-5">
            <form method="POST" action="{{ route('super-admin.administradores.update', $usuario->id) }}" class="space-y-5" x-data="{ tieneMasRoles: {{ old('tiene_mas_roles', count($currentAdditionalRoles) > 0) ? 'true' : 'false' }}, mostrarGrupoInvestigacion: {{ in_array('co_investigador_gdi', old('additional_roles', $currentAdditionalRoles), true) ? 'true' : 'false' }} }">
                @csrf
                @method('PUT')
