<x-app-layout>
    <x-slot name="header">Nuevo administrador del sistema</x-slot>

    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-xl font-semibold text-slate-900">Nuevo administrador del sistema</h2>
            <p class="text-sm text-slate-500 mt-1">El usuario se creará con el rol <strong>administrador_sistema</strong>. Puedes asignarle un centro ahora o dejarlo sin centro y vincularlo después.</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden max-w-3xl">
        ...
        <div class="p-5">
            <form method="POST" action="{{ route('super-admin.administradores.store') }}" class="space-y-5" x-data="{ tieneMasRoles: {{ old('tiene_mas_roles') ? 'true' : 'false' }}, mostrarGrupoInvestigacion: {{ in_array('co_investigador_gdi', old('additional_roles', []), true) ? 'true' : 'false' }} }">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="md:col-span-2">
                        <label for="training_center_id" class="block text-sm font-medium text-slate-700 mb-1.5">
                            Centro de formación <span class="text-slate-400 font-normal">(opcional — puede vincularse después)</span>
                        </label>
                        <select name="training_center_id" id="training_center_id"
                            ...
                            <option value="">— Sin centro asignado por ahora —</option>
                        ...
                        <p class="text-xs text-slate-500 mt-1">
                            Si dejas sin centro, podrás vincularlo desde <strong>Centro ↔ administrador</strong>.
                        </p>
                    </div>
                    ...
                    <div x-show="tieneMasRoles && mostrarGrupoInvestigacion" x-cloak class="mt-3.5 pt-3.5 border-t border-slate-200">
                        ...
                        <select name="grupo_investigacion_id" id="grupo_investigacion_id" class="w-full border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm text-slate-800 bg-white focus:border-[#39A900] focus:ring-2 focus:ring-[#39A900]/10">
                            ...
                            @foreach($gruposInvestigacion as $g)
                                <option value="{{ $g->id }}" {{ (string) old('grupo_investigacion_id') === (string) $g->id ? 'selected' : '' }}>{{ $g->nombre }} ({{ $g->trainingCenter?->nombre }})</option>
                            @endforeach
                        </select>
                        <p class="text-xs text-slate-500 mt-1">Debe pertenecer al mismo centro de formación que elijas arriba.</p>
                ...
                <div class="rounded-lg border border-slate-200 bg-slate-50/60 px-4 py-3.5 flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mt-0.5">Se enviará al administrador su número de documento y contraseña al correo registrado.</p>
                    </div>
                </div>
