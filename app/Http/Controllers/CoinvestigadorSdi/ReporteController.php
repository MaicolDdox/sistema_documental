...
class ReporteController extends Controller
{
    public function descargar()
    {
        ...
        $vinculaciones = $usuario->projectAuthors()
            ...
            ->with([
                'project.seedling',
                'project.liderProyecto.person',
                'project.evidenciasProductoFinal' => fn ($q) => $q->where('estado_revision_director', \App\Enums\EstadoRevisionEnum::Aprobado),
            ])
            ->get();

        $filas = $vinculaciones->map(function ($va) {
            $proyecto = $va->project;
            $productosAprobados = $proyecto ? $proyecto->evidenciasProductoFinal->count() : 0;
        ...
        $pdf = Pdf::loadView('co_investigador_sdi.reportes.pdf.mis_proyectos', [
            'filas' => $filas,
            'meta' => $meta,
        ]);
