...
class ProfileController extends Controller
{
    ...
    public function edit()
    {
        ...
        // "centro incorrecto" posible para su perfil, así que ve todas las
        // filas sin filtro. El resto de roles solo ve las de su centro.
        $entityPositionsQuery = EntityPosition::query();
        $linkageTypesQuery = LinkageType::query();
        if ($user->training_center_id !== null) {
            $entityPositionsQuery->where('training_center_id', $user->training_center_id);
            $linkageTypesQuery->where('training_center_id', $user->training_center_id);
        }

        return view('settings.profile-page', [
            'user' => $user,
            'person' => $person,
            'entityPositions' => $entityPositionsQuery->orderBy('nombre')->get(),
            'linkageTypes' => $linkageTypesQuery->orderBy('nombre')->get(),
